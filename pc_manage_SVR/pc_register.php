...
$pageNumber = 0;
if (isset($_SESSION['parameters_pc'])) {
	//$pageNumberのみ値変更して書き込み
//	$filename = "..\parameters.txt";
	$mergedData = $_SESSION['parameters_pc'];
	...
	$kwdMakerPrev = $kwd[8];
	$kwdModelPrev = $kwd[9];
	$kwdCarryOutPrev = $kwd[10];
	$kwdRentalTerminationPrev = $kwd[11];
...
$mergedData = $pageNumber.'#'.$KwdSetStatus.'#'.$searchType.'#'.$kwdCompanyPrev.'#'.$kwdOfficePrev.'#'.
              $kwdRegisterNoPrev.'#'.$kwdUserPrev.'#'.$kwdPlacePrev.'#'.$kwdMakerPrev.'#'.$kwdModelPrev.'#'.$kwdCarryOutPrev.'#'.
              $kwdRentalTerminationPrev.'#'.$orderType.'#'.$sortRule;
$_SESSION['parameters_pc'] = $mergedData;
...
//			if ($orderType == ""){ $orderType = 'orderAsc';} else { $orderType = $kwd[11];}
    ...
    //<!-- 戻るボタンのフォーム -->
  			echo " <form method=\"POST\" action=\"pc_register.php\">";
...
// parameters_pc.txtファイルがあれば読み出し、なければ初期値を設定してファイル生成
...
// var_dump($kwdCompany);echo "<br/>\n";
 ...
 do{
  ...
  $kwdUser = preg_replace('/^[ 　]+/u', '', es($_POST["kwdUser"]));
  $kwdUser = preg_replace('/[ 　]+$/u', '', $kwdUser);
  $GLOBALS['kwdUserPrev'] = $kwdUser;
  $kwdUser = (array)setKwd($GLOBALS['kwdUserPrev']);
 $i = 0;
 do{
  	$kwdUser[$i] =preg_replace('/^[ 　]+/u', '', $kwdUser[$i]);
  	$kwdUser[$i] = preg_replace('/[ 　]+$/u', '', $kwdUser[$i]);
  	$i ++;
  } while ($i < count($kwdUser));
...
if ($GLOBALS['searchType'] === "andSearch"){
    ...
    if ($kwdUser[0] !== ""){
		foreach ($kwdUser as $row) {
			 if ( $sql == ""){ 
			 	$sql = " SELECT * FROM $table_list WHERE((利用者 COLLATE $collateType LIKE '%". $row ."%')";
			 	$positionIndicator = 1;
			 }else { 
			 	if ($positionIndicator == 1){
			 		$sql = $sql."OR (利用者 COLLATE $collateType LIKE '%". $row ."%')"; 
			 	} else{
			 		$sql = $sql."AND ((利用者 COLLATE $collateType LIKE '%". $row ."%')"; 
				 	$positionIndicator = 1;
			 	}
	 		}
		} 
    ...
    if ($kwdRentalTermination[0] != ""){
		foreach ($kwdRentalTermination as $row) {
			 if ( $sql == "") { 
			 	$sql = " SELECT * FROM $table_list WHERE((レンタル終了日 COLLATE $collateType LIKE '%". $row ."%')";
			 	$positionIndicator = 1;
			 }else { 
			 	if ($positionIndicator == 1){
			 		$sql = $sql."OR (レンタル終了日 COLLATE $collateType LIKE '%". $row ."%')"; 
			 	} else{
			 		$sql = $sql."AND ((レンタル終了日 COLLATE $collateType LIKE '%". $row ."%')"; 
				 	$positionIndicator = 1;
			 	}
	 		}
		} 
...
// 並べ替え　$sortRule(会社、メーカー名等)、$orderType(昇順(ASC)降順(DESC))
if ($sortRule !== "") {
...
if ($GLOBALS['searchType'] === "orSearch"){
  ...
  foreach($temp_records as $key => $row){
   		...
   		echo "<td class=\"th0\">", es($row['会社']), "</td>";
		echo "<td class=\"th3\">", es($row['事業所']), "</td>";
    	echo "<td class=\"th1\">", es($row['部署']), "</td>";
    	echo "<td class=\"th2\">", es($row['台帳番号']), "</td>";
    	echo "<td class=\"th8\">", es($row['利用者']), "</td>";
...
//    	echo "<td class=\"th6\">", "<a href=",'"mailto:',es($row['持出可否']),'"',">",es($row['持出可否']),"</a>", "</td>";
    	echo "<td class=\"th7\">", es($row['レンタル終了日']), "</td>";
 ...
 echo "</div>";
	...
	if ($originalkwdUser != 'no_keyword'){
	  echo "<li>";echo "<span>"; echo  "検索キーワード(台帳番号)：　";
	  $i = 0;
	  do{ echo $originalkwdUser[$i], '&nbsp; &nbsp; &nbsp; ';$i ++;
	  }while(count((array)$originalkwdUser)> $i );
	  echo "</span>";echo "</li>";
	} else {
	  echo  "<li>";echo "<span>"; echo  "検索キーワード(台帳番号):なし","</span>";echo "</li>";	
	}
	...
	if ($originalkwdRentalTermination != 'no_keyword'){
	  echo "<li>";echo "<span>"; echo  "検索キーワード(レンタル終了日)：　";
	  $i = 0;
	  do{ echo $originalkwdRentalTermination[$i], '&nbsp; &nbsp; &nbsp; ';$i ++;
	  }while(count((array)$originalkwdRentalTermination)> $i );
	  echo "</span>";echo "</li>";
	} else {
	  echo  "<li>";echo "<span>"; echo  "検索キーワード(レンタル終了日):なし","</span>";echo "</li>";	
	}
	  echo '</font></ul>';
...
<?php
 	echo '<title>PC管理情報検索ツール</title>';
 	echo '<!-- テーブル用のスタイルシート -->';
 	echo '<link rel="stylesheet" type="text/css" href="style2.css" >'; 
 	echo '<form method="POST" action="" >';
 	echo '<label><div>Welcome to</div></label>';
 	echo '<div><h1 class="title">PC管理情報検索</h1>'; 
 	echo '<ul class="comment">';
        echo ' <li>全社で一括管理する情報系のPC管理情報を検索できるサービスです。</li>';
        echo ' <li>検索項目夫々に複数入力（会社なら「TRK、KTS」のように）が可能です。<br>';
        echo ' 【例１】「検索タイプ：◎ANDサーチ」「部署：新潟工場、中条工場」「メーカー：Lenovo」を選択すると新潟工場ま<br>';
        echo '&emsp;&emsp;&emsp;&emsp;たは中条工場の全てのLenovo製PCが検索されます。<br>';

        echo ' 【例２】「検索タイプ：◎ORサーチ」いずれか１つの検索項目（会社、部署、・・・）にのみ検索キーワードを入力<br>';
        echo ' 　　　　して利用ください。「部署：大阪営業所、京都営業所」を選択すると、両営業所の全PCが検索されます。';
        echo ' </li>';
...
<option value="会社" align="center"';selected("会社", $GLOBALS['sortRule']);echo '>会社</option>
<option value="事業所" align="center"';selected("事業所", $GLOBALS['sortRule']);echo '>事業所</option>
<option value="部署" align="center"';selected("部署", $GLOBALS['sortRule']);echo '>部署</option>
<option value="台帳番号" align="center"';selected("台帳番号", $GLOBALS['sortRule']);echo '>台帳番号</option>
<option value="利用者" align="center"';selected("利用者", $GLOBALS['sortRule']);echo '>利用者</option>
...
<option value="レンタル終了日" align="center"';selected("レンタル終了日", $GLOBALS['sortRule']);echo '>レンタル終了日</option></font>';
...
if ($GLOBALS['kwdSetStatus'] != "kwdSet"){
 	echo '<tr><th>会社</th><td><input type="text" name="kwdCompany" style="border:none;font-size:20px;" placeholder="ATK、KTS etc." size=40></td></tr>';
 	echo '<tr><th>事業所</th><td><input type="text" name="kwdPlace" style="border:none;font-size:20px;" placeholder="新潟工場、本社 etc." size=40></td></tr>';
 	echo '<tr><th>部署</th><td><input type="text" name="kwdOffice" style="border:none;font-size:20px;" placeholder="***営業所、海外営業部、カートリッジ製造G etc." size=40></td></tr>';
 	...
 	echo '<tr><th>利用者</th><td><input type="text" name="kwdUser" style="border:none;font-size:20px;" placeholder="東洋　太郎" size=40></td></tr>';
 	...
 	echo '<tr><th>持出可否</th><td><input type="text" style="border:none;font-size:20px;" name="kwdCarryOut" placeholder="持出可" size=40></td></tr>';
 	echo '<tr><th>レンタル終了日</th><td><input type="text" style="border:none;font-size:20px;" name="kwdRentalTermination" placeholder="2025/3/31 etc." size=40></td></tr>';
} else { 
 	...
 	echo '<tr><th>事業所</th><td><input type="text" name="kwdPlace" style="border:none;font-size:20px;" size=40 value=',$GLOBALS['kwdPlacePrev'],'></td></tr>';
 	echo '<tr><th>部署</th><td><input type="text" name="kwdOffice" style="border:none;font-size:20px;"size=40 value=',$GLOBALS['kwdOfficePrev'],'></td></tr>'; 
 	...
 	echo '<tr><th>利用者</th><td><input type="text" name="kwdUser" style="border:none;font-size:20px;" size=40 value=',$GLOBALS['kwdUserPrev'],'></td></tr>';
 	...
 	echo '<tr><th>レンタル終了日</th><td><input type="text" style="border:none;font-size:20px;" name="kwdRentalTermination" size=40 value=',$GLOBALS['kwdRentalTerminationPrev'],'></td></tr>';
}
 	echo '</table>';
 	echo '</form>';
}
...
//SQLサーバに接続する
  ...
  // 値を取り出して行に表示する
    ...
    // １行ずつテーブルに入れる
    	...
    	echo "<td class=\"th1\">", es($row['部署']), "</td>";
    	echo "<td class=\"th2\">", es($row['台帳番号']), "</td>";
	  	echo "<th class=\"th8\">", es($row['利用者']), "</th>";
    	echo "<td class=\"th4\">", es($row['メーカー']), "</td>";
    	echo "<td class=\"th5\">", es($row['型式']), "</td>";
    	echo "<td class=\"th6\">", es($row['持出可否']), "</td>";
    	echo "<td class=\"th7\">", es($row['レンタル終了日']), "</td>";
